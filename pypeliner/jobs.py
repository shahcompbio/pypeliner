...
class JobCallable(object):
    ...
    def getmounts(self):
        mounts = set()
        for arg in self.arglist:
            mount = arg.get_mount_point()
            if not mount:
                continue
            if isinstance(mount, list):
                mounts = mounts.union(mounts)
            else:
                mounts.add(mount)
    ...
    def __call__(self):
        ...
        if self.func == pypeliner.commandline._docker_python_execute:
            ...
            callset.args[-2] = mounts
        elif self.func == pypeliner.commandline.execute:
            callset.kwargs = {} if not callset.kwargs else callset.kwargs
            callset.kwargs['mounts'] = mounts
